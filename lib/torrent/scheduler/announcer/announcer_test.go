...
func TestAnnouncerAnnounceUpdatesInterval(t *testing.T) {
	...
	mocks.client.EXPECT().Announce(d, hash, false, nil, nil, announceclient.V3).Return(peers, interval, nil)

	result, err := announcer.Announce(d, hash, false, nil, nil)
	require.NoError(err)
...
func TestAnnouncerAnnounceErr(t *testing.T) {
	...
	mocks.client.EXPECT().Announce(d, hash, false, nil, nil, announceclient.V3).Return(nil, time.Duration(0), err)

	_, aErr := announcer.Announce(d, hash, false, nil, nil)
