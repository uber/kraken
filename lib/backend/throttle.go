...
func (c *ThrottledClient) Upload(namespace, name string, src io.Reader) error {
	if s, ok := src.(sizer); ok {
		...
		log.With("name", name, "error", "cannot determine size of blob to upload").Errorf("Could not self-throttle during remote blob upload. Proceeding with no throttling to fail-open")
	}
