...
const (
	...
	_membershipWaitMetric  = "hashring_membership_wait_duration"
)
...
func New(
	...
	bucketWidth := config.MembershipWaitTimeout / _membershipWaitBuckets
	buckets := tally.MustMakeLinearDurationBuckets(0, bucketWidth, _membershipWaitBuckets)
	r := &ring{
		config:                 config,
		cluster:                cluster,
		filter:                 filter,
