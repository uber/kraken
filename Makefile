...
CROSS_COMPILER = docker run --rm -v $(shell pwd):/go/src/github.com/uber/kraken -w /go/src/github.com/uber/kraken -e GO111MODULE=on -e GIT_SSL_NO_VERIFY=true -e GOPROXY=$(GOPROXY) -e GOSUMDB=off -e GOINSECURE="*" $(GOLANG_IMAGE) go build -o ./$@ ./$(dir $@)
...
images: $(LINUX_BINS)
	...
	docker build --no-cache $(BUILD_QUIET) -t kraken-origin:$(PACKAGE_VERSION) -f docker/origin/Dockerfile ./
