...
# Origin acceptances tests will place the generated image tag in this file.
TEST_TAG_OUTFILE = $$TMPDIR/latest-kraken-test-tag.out

# If tag variable is set, use it. Else, use a tag generated by origin tests.
GET_TAG = $(shell if [[ -z "$(tag)" ]]; then cat $(TEST_TAG_OUTFILE); else echo "$(tag)"; fi)
...
acceptance/origin:
	...
	ssh $(host) bash < ./test/acceptance/proxy.sh | tee out
	grep -E 'TAG=.*' out | awk -F'=' '{print $$2}' | tail -n 1 > $(TEST_TAG_OUTFILE)
	rm out
...
acceptance/tracker:
	...
	ssh $(host) "bash -s $(GET_TAG)" < ./test/acceptance/build-index.sh
	ssh $(host) "bash -s $(GET_TAG)" < ./test/acceptance/tracker.sh

# Runs acceptance tests on an agent host.
acceptance/agent:
	@test $(host)
	ssh $(host) 'bash -s agent' < ./test/acceptance/health.sh
	ssh $(host) "bash -s $(GET_TAG)" < ./test/acceptance/agent.sh
