...
# container (containerd + soci-snapshotter + nerdctl/soci/ctr), which runs the
# overlayfs-vs-soci comparison (run_e2e.sh) as its own workload. The host streams
# the results via `docker logs -f` -- deliberately NOT `docker exec`, which needs
# an HTTP->TCP connection upgrade that the busy daemon transiently refuses with
# "unable to upgrade to tcp, received 200". `docker logs` is a plain stream.
#
...
# only THIS run's blob fetches. The soci (lazy) leg hits agent-two with 206
# ranged GETs; the overlayfs (full) leg hits agent-one with 200 GETs. Comparing
# bytes fetched is the clearest lazy proof -- soci should transfer far less than
# the full image. Agent log line: '... " 206 <bytes> "" "<ua>"' (bytes = NF-2).
...
"${SOCI_DIR}/soci_start_container.sh" "${IMAGE}" "${CMD[@]}"
...
echo "soci      (lazy, agent-two 206): $(blob_stats kraken-agent-two "${a2_start}" 206)" >&2
echo "overlayfs (full, agent-one 200): $(blob_stats kraken-agent-one "${a1_start}" 200)" >&2
echo "soci should be FAR below overlayfs; if not, the layer was read in full" >&2
echo "====================================================================" >&2
